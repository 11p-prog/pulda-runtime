...
> **현재 경계 (2026-07-16)**: Event·상태 저장과 규칙 기반 분류·순환은 구현되어
> 있습니다. 실제 AI/LLM 해석 계층은 **연결되지 않았습니다.** Living Loop(AI 해석
> → 인간 교정 → 결과 → 후속 제안)는 미완성입니다. 모바일 입력, 인간 검토 UI,
> 실제 사용자 Event 검증은 AI 연결 전이므로 아직 수행하지 않았습니다.
> Runtime 실행 가능과 Pulda Living Loop 완성을 동일하게 표현하지 않습니다.
> 다음 후보 작업: 실제 AI 해석 계층 연결.
...
- AI 또는 connector 장애가 나도 원본 Event는 유실하지 않고 Pending으로 보존

## 현재 한계

- Telegram 등 모바일 메시징 직접 수집 없음
- LLM 없이 규칙 기반 분류
- 해석·교정·결과·후속 제안은 현재 API로만 제공되며 사용자 UI는 미구현
